package com.khansuf.lonkan.utils;

public class Facts
{

	public static String[] EnglishFact = {
			...
			"If you don’t flush a public toilet after yourself in Singapore and get caught, expect a fine of at least 150 USD, and that’s only if you’re a first-time offender.",
	...
	public static String[] BanglaFact = {
	...
	public static String getEnglisFactByIndex(int index) {
		try {
			if (EnglishFact.length <= index) return EnglishFact[EnglishFact.length - 1];
			return EnglishFact[index];
		} catch (Exception e) {
			return e.toString();
		}
	}

	public static String getBanglaFactByIndex(int index) {
		try {
			if (BanglaFact.length <= index) return BanglaFact[BanglaFact.length - 1];
			return BanglaFact[index];
		} catch (Exception e) {
			return e.toString();
		}
	}
